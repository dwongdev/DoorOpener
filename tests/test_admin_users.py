...
def test_admin_users_create(mock_users_store):
    ...
    response = c.post(
        "/admin/users", json={"username": "newuser", "pin": "1234", "active": True}, headers=_csrf_headers()
    )
...
def test_admin_users_create_duplicate(mock_users_store):
    ...
    response = c.post(
        "/admin/users", json={"username": "existing", "pin": "5678", "active": True}, headers=_csrf_headers()
    )
